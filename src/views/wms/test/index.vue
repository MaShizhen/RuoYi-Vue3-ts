        ...
        <el-row>
          ...
          <el-col :span="12">
            ...
            <el-form-item label="有效天数" prop="validDays">
              <el-input-number :min="1" v-model="form.validDays" />
            </el-form-item>
...
<script setup name="Material-type">
import {
  listMaterialType,
  addMaterialType,
  delMaterialType,
...
const data = reactive({
  ...
  rules: {
    ...
    prefixNo: [
      { required: true, message: "编码前缀不能为空", trigger: "blur" },
    ],
...
function getList() {
  ...
  listMaterialType(queryParams.value).then((response) => {
    const dataRows = response.data.map((i) => {
      return {
        ...i,
        value: i.typeId,
        label: i.typeName,
...
function handleDelete(row) {
  ...
  const ids = deleteList.value.map((i) => i.typeId);
  const typeNos = deleteList.value.map((i) => i.typeNo);
  proxy.$modal
...
function toogleSelectWarehouse(bool) {
  selectWarehouse.value = bool;
}
function onSelectWarehouse(row) {
